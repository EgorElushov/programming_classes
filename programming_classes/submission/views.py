...
class SubmissionListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # Filter by the current user unless user is staff or viewing specific other user
...
class SubmissionDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_object(self):
        ...
        if not (self.request.user == submission.user or self.request.user.is_staff):
            # Check if user is a competition admin for this problem
            problem = submission.problem
            competition = problem.competition
            if self.request.user != competition.user:
                raise HttpResponseForbidden("You don't have permission to view this submission")
    ...
    def get_context_data(self, **kwargs):
        ...
        context['competition'] = submission.problem.competition
        
        # Check if the current viewer is the owner of the submission
        context['is_owner'] = (self.request.user == submission.user)
        
        # Add permission to manage submission status (for staff or competition admin)
        context['can_manage'] = (self.request.user.is_staff or 
                                self.request.user == submission.problem.competition.user)
        
        return context

@login_required
def create_submission(request, problem_id):
    """View function for creating a new submission"""
    ...
    if not is_active and not request.user.is_staff:
        messages.error(request, "This competition is not active. Submissions are closed.")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            submission.status = 'pending'  # Initial status
            submission.save()
            
            messages.success(request, "Your submission has been received and is being processed.")
            
            # Redirect to submission detail page
            return redirect('submission-detail', submission_id=submission.submission_id)
    else:
        # Pre-fill the form with empty code
        form = SubmissionForm()
...
def update_submission_status(request, submission_id):
    ...
    if not (request.user.is_staff or request.user == submission.problem.competition.user):
        return HttpResponseForbidden("You don't have permission to update this submission's status")
...
def user_submissions(request, user_id=None):
    """View function to see a specific user's submissions (staff only) or own submissions"""
    # If user_id is not provided, show current user's submissions
    if user_id is None:
        user_id = request.user.id
    
    # Check permissions - only staff can view other users' submissions
    if int(user_id) != request.user.id and not request.user.is_staff:
        return HttpResponseForbidden("You don't have permission to view this user's submissions")
    ...
    # Get the list of problems the user has solved
    solved_problems = Problem.objects.filter(
        submissions__user=user,
        submissions__status='accepted'
    ).distinct()
    
    return render(request, 'submissions/user_submissions.html', {
        'user': user,
...
def problem_submissions(request, problem_id):
    """View all submissions for a specific problem"""
    problem = get_object_or_404(Problem, problem_id=problem_id)
    
    # Check if user has permission to view all submissions for this problem
    ...
    return render(request, 'submissions/problem_submissions.html', {
        'problem': problem,
        'submissions': submissions,
        'stats': stats,
        'is_admin': is_admin,
...
def competition_submissions(request, competition_id):
    """View all submissions for a specific competition"""
    competition = get_object_or_404(Competition, id=competition_id)
    
    # Check if user has permission to view all submissions for this competition
    ...
    if is_admin:
        ...
        # Get submission statistics by problem
        problem_stats = []
        ...
        # Get submission statistics by user
        user_stats = Submission.objects.filter(problem__in=problems).values(
        ...
        # Regular users can only see their own submissions for this competition
        problems = Problem.objects.filter(competition=competition)
...
def submission_statistics(request):
    ...
    # Most active users
    most_active_users = Submission.objects.values(
        'user__id', 'user__username'
    ).annotate(
        submission_count=Count('submission_id')
    ).order_by('-submission_count')[:10]
    
    return render(request, 'submissions/submission_statistics.html', {
...
# API endpoints for automated checking
...
def submit_solution_api(request, problem_id):
    ...
    try:
        ...
        if not is_active and not request.user.is_staff:
            return JsonResponse({'error': 'This competition is not active'}, status=403)
...
def check_submission_status_api(request, submission_id):
    """API endpoint to check a submission's status"""
    try:
        ...
        if not (request.user == submission.user or 
                ...
                request.user == submission.problem.competition.user):
            return JsonResponse({'error': 'You do not have permission to view this submission'}, status=403)
...
def update_submission_status_api(request, submission_id):
    """API endpoint to update a submission's status (admin/staff only)"""
    if request.method != 'POST':
        return JsonResponse({'error': 'Only POST method is allowed'}, status=405)
    ...
    try:
        ...
        # Check if user has permission to update this submission
        if not (request.user.is_staff or request.user == submission.problem.competition.user):
            return JsonResponse({'error': 'You do not have permission to update this submission'}, status=403)
...
def bulk_grading(request):
    """View for bulk grading multiple submissions at once (staff only)"""
    if not request.user.is_staff:
    ...
    return render(request, 'submissions/bulk_grading.html', {
...
def automated_grader_webhook(request):
    ...
    if not api_key or api_key != settings.GRADER_API_KEY:  # You'd need to define this in settings
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({
                'success': True,
                'message': f'Submission {submission_id} updated to status: {new_status}'
            })
...
def submission_leaderboard(request, competition_id):
    ...
    leaderboard = Submission.objects.filter(
        ...
        last_accepted=models.Max('submitted_at')
