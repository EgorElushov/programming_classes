...
class ProblemDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            context['can_edit'] = (self.request.user == problem.competition.user or 
...
class ProblemCreateView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.competition_id:
            ...
            if not (request.user == self.competition.user or request.user.is_staff):
                return HttpResponseForbidden("You don't have permission to add problems to this competition")
    ...
    def get_success_url(self):
        if hasattr(self, 'competition_id') and self.competition_id:
            return reverse('competition-problems', kwargs={'pk': self.competition_id})
...
def competition_problems(request, pk):
    ...
    return render(request, 'problems/competition_problems.html', {
...
def bulk_problem_upload(request, competition_id):
    ...
    if request.method == 'POST':
        if 'problem_file' in request.FILES:
            try:
                ...
                messages.success(request, f"{problems_created} problems were successfully imported")
    ...
    return render(request, 'problems/bulk_problem_upload.html', {
        'competition': competition
    })

@login_required
def problem_batch_operations(request, competition_id):
    """Perform batch operations on problems"""
    competition = get_object_or_404(Competition, pk=competition_id)
    
    # Check permissions
    if not (request.user == competition.user or request.user.is_staff):
        return HttpResponseForbidden("You don't have permission to modify problems in this competition")
    ...
    if request.method == 'POST':
        ...
        if not problem_ids:
            messages.error(request, "No problems selected")
            return redirect('competition-problems', pk=competition_id)
        ...
        if operation == 'delete':
            ...
            problems.delete()
            messages.success(request, f"{count} problems deleted successfully")
            
        elif operation == 'change_difficulty':
            new_difficulty = request.POST.get('new_difficulty')
            if new_difficulty:
                problems.update(difficulty=new_difficulty)
                messages.success(request, f"Difficulty updated for {problems.count()} problems")
            else:
                messages.error(request, "New difficulty level not specified")
                
        return redirect('competition-problems', pk=competition_id)
    
    return render(request, 'problems/problem_batch_operations.html', {
        'competition': competition,
...
def export_problems_api(request, competition_id):
    """Export problems in JSON format for external use"""
    competition = get_object_or_404(Competition, pk=competition_id)
...
def import_problems_api(request, competition_id):
    """Import problems from an external source via API"""
    competition = get_object_or_404(Competition, pk=competition_id)
    ...
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({
                'success': True,
                'message': f'{created_count} problems imported successfully'
            })
