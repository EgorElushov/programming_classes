...
class CourseDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['recent_enrollments'] = Enrollment.objects.filter(course=course).order_by('-enrolled_at')[:5]

        context['materials'] = CourseMaterial.objects.filter(course=course)
...
def enroll_course(request, course_id):
    ...
    messages.success(request, f"Вы успешно записаны на {course.title}!")
...
def unenroll_course(request, course_id):
    ...
    try:
        ...
        enrollment.delete()
        messages.success(request, f"Вы отписались от {course.title}.")
    except Enrollment.DoesNotExist:
        messages.error(request, "Вы еще не записаны на этот курс.")
...
def my_courses(request):
    ...
    enrolled_courses = Course.objects.filter(enrollments__user=request.user).exclude(created_by=request.user)

    return render(
        request, 'courses/my_courses.html', {'created_courses': created_courses, 'enrolled_courses': enrolled_courses}
    )
...
def course_enrollments(request, course_id):
    ...
    if not (request.user == course.created_by or request.user.is_staff):
        return HttpResponseForbidden("У вас нет разрешения просматривать записи на этот курс")

    enrollments = Enrollment.objects.filter(course=course).order_by('-enrolled_at')

    return render(
        request,
        'courses/course_enrollments.html',
        {'course': course, 'enrollments': enrollments, 'enrollment_count': enrollments.count()},
    )
...
def manage_enrollment(request, enrollment_id, action):
    ...
    if not (request.user == course.created_by or request.user.is_staff):
        return HttpResponseForbidden("У вас нет прав на управление записями на этот курс")

    if action == 'remove':
        user = enrollment.user
        enrollment.delete()
        messages.success(request, f"{user.username} успешно отчислен с курса {course.title}.")
...
def add_students_to_course(request, course_id):
    ...
    if not (request.user == course.created_by or request.user.is_staff):
        return HttpResponseForbidden("У вас нет разрешения добавлять студентов на этот курс")
    ...
    if request.method == 'POST':
        ...
        if not student_identifiers:
            messages.error(request, "Не указано ни одного студента.")
        ...
        if enrolled_count > 0:
            messages.success(request, f"{enrolled_count} студент(ы) успешно зачислен(ы).")
        if already_enrolled_count > 0:
            messages.info(request, f"{already_enrolled_count} student(s) were already enrolled.")
        if not_found_count > 0:
            messages.error(request, f"{not_found_count} не удалось найти студента(ов).")

        return redirect('course-enrollments', course_id=course_id)

    return render(request, 'courses/add_students.html', {'course': course})
...
def course_statistics(request, course_id):
    ...
    if not (request.user == course.created_by or request.user.is_staff):
        return HttpResponseForbidden("У вас нет разрешения на просмотр статистику по этому курсу")
    ...
    material_types = materials.values('file_type').annotate(count=Count('id')).order_by('file_type')

    return render(
        request,
        'courses/course_statistics.html',
        {
            'course': course,
...
def course_materials(request, course_id):
    ...
    if not (is_enrolled or is_creator or request.user.is_staff):
        return HttpResponseForbidden("Вы должны быть зарегистрированы на этом курсе, чтобы ознакомиться с материалами")

    materials = CourseMaterial.objects.filter(course=course).order_by('title')

    return render(
        request, 'courses/course_materials.html', {'course': course, 'materials': materials, 'is_creator': is_creator}
    )
    ...
    courses_data = [
        {
            ...
            'created_by': {'id': course.created_by.id, 'username': course.created_by.username},
...
def enroll_course_api(request, course_id):
    if request.method != 'POST':
        return JsonResponse({'error': 'Доступно только POST метод'}, status=405)

    try:
        course = Course.objects.get(course_id=course_id)

        if Enrollment.objects.filter(user=request.user, course=course).exists():
            return JsonResponse({'error': 'Уже записан на курс'}, status=400)
...
def unenroll_course_api(request, course_id):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({'success': True, 'message': f'Успешно отписался от {course.title}'})

        except Enrollment.DoesNotExist:
            return JsonResponse({'error': 'Нет записи на курс'}, status=400)
...
def create_course_api(request):
    ...
    try:
        ...
        if not title:
            return JsonResponse({'error': 'Название курса не может быть пустым'}, status=400)
