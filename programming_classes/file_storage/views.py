...
class CourseMaterialDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_object(self):
        ...
        if not (is_enrolled or is_creator or self.request.user.is_staff):
            raise HttpResponseForbidden("You don't have permission to access this material")
    ...
    def get_context_data(self, **kwargs):
        ...
        context['can_edit'] = (self.request.user == material.uploaded_by or 
                              self.request.user == material.course.created_by or 
...
def download_material(request, pk):
    ...
    if not (is_enrolled or is_creator or request.user.is_staff):
        return HttpResponseForbidden("You don't have permission to download this material")
...
class CourseMaterialCreateView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not (request.user == self.course.created_by or request.user.is_staff):
            return HttpResponseForbidden("You don't have permission to add materials to this course")
...
class CourseMaterialUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self):
        material = self.get_object()
        return (self.request.user == material.uploaded_by or 
                self.request.user == material.course.created_by or 
                self.request.user.is_staff)
...
def bulk_upload_materials(request, course_id):
    ...
    if not (request.user == course.created_by or request.user.is_staff):
        return HttpResponseForbidden("You don't have permission to upload materials to this course")
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f"{count} materials uploaded successfully!")
        return redirect('course-materials', course_id=course_id)
    
    return render(request, 'file_storage/bulk_upload.html', {
        'course': course
    })
...
def material_preview(request, pk):
    ...
    if not (is_enrolled or is_creator or request.user.is_staff):
        return HttpResponseForbidden("You don't have permission to preview this material")
    ...
    if material.file_type == 'document' and file_ext in ['pdf']:
        ...
        return render(request, 'file_storage/video_preview.html', {
            'material': material
        })
        
    elif material.file_type == 'audio':
        return render(request, 'file_storage/audio_preview.html', {
            'material': material
        })
        
    else:
        messages.info(request, "Preview not available for this file type. Downloading instead.")
...
def course_materials_api(request, course_id):
    ...
    try:
        ...
        if not (is_enrolled or is_creator or request.user.is_staff):
            return JsonResponse({'error': 'You do not have permission to access these materials'}, status=403)
...
def upload_material_api(request, course_id):
    """API endpoint to upload materials to a course"""
    if request.method != 'POST':
    ...
    try:
        ...
        if not (request.user == course.created_by or request.user.is_staff):
            return JsonResponse({'error': 'You do not have permission to upload materials to this course'}, status=403)
...
def delete_material_api(request, pk):
    ...
    try:
        ...
        if not (request.user == material.uploaded_by or 
                request.user == material.course.created_by or 
                request.user.is_staff):
            return JsonResponse({'error': 'You do not have permission to delete this material'}, status=403)
...
def material_statistics(request, course_id):
    """View statistics about materials for a course"""
    course = get_object_or_404(Course, course_id=course_id)
    ...
    materials_by_uploader = materials.values(
    ...
    return render(request, 'file_storage/material_statistics.html', {
        'course': course,
...
def serve_material(request, pk):
    """Serve material file for streaming (video/audio)"""
    material = get_object_or_404(CourseMaterial, pk=pk)
    course = material.course
    ...
    if not (is_enrolled or is_creator or request.user.is_staff):
        return HttpResponseForbidden("You don't have permission to access this material")
    ...
    if range_header:
        ...
        if range_match:
            ...
            response = FileResponse(
                RangeFileWrapper(open(file_path, 'rb'), start, length),
